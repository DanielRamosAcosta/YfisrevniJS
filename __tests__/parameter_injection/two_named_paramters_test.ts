...
import {
  ...
  GREETER_SYMBOL,
  GREETER_TAGS,
} from "./example_classes/greeter.ts";
...
Deno.test("two named parameters injection", () => {
  ...
  myContainer.bind<Greeter>(GREETER_SYMBOL).to(GreeterEnglish).whenTargetNamed(
    GREETER_TAGS.USA,
  );
  myContainer.bind<Greeter>(GREETER_SYMBOL).to(GreeterSpanish).whenTargetNamed(
    GREETER_TAGS.Spain,
  );
